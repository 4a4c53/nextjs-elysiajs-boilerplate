...
type BadgeProps = Prettify<useRender.ComponentProps<'span'> & VariantProps<typeof BADGE_VARIANTS>>
...
function Badge({ className, variant = 'default', render, ...props }: BadgeProps) {
	return useRender({
		...
		props: mergeProps<'span'>(
			{
				className: cn(BADGE_VARIANTS({ className, variant })),
			},
			props,
		),
		render,
